#!/bin/sh
# Runs from /srv (the data volume: gun's radata and the https certificate
# land here) but executes the app baked into the image at /app.
#
...
# With MODE=https, generates a self-signed certificate for that address on
# first start (kept in the volume, so a restart doesn't regenerate it).
set -eu
cd /srv
...
if [ "${MODE:-http}" = https ]; then
  ...
  if [ ! -f "$HTTPS_KEY" ]; then
    openssl req -x509 -newkey rsa:2048 -nodes -keyout "$HTTPS_KEY" -out "$HTTPS_CERT" \
      -days 3650 -subj '/CN=gun-relay' -addext "subjectAltName=IP:$RELAY_IP,DNS:localhost"
  fi
fi
