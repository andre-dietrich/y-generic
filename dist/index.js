...
export class GenericProvider extends Observable {
     ...
     */
    _scheduleSyncReply(reply, isAck = false, targetSv = null, requester = null, late = false) {
    ...
    _replyToSyncRequest(reply, isAck = false, targetSv = null, toClientID) {
        ...
        const incomplete = this.doc.store.pendingStructs !== null || this.doc.store.pendingDs !== null;
        if (incomplete && isAck)
            return;
        ...
        if (toClientID !== undefined && this._canUnicast(toClientID)) {
            ...
            if (incomplete) {
                ...
                setTimeout(() => {
                    if (!this._destroying && this._tryReserveReplySlot())
            ...
            for (let i = 0; i < len; i++) {
                ...
                if (state === null)
                    ...
                    if (clientID === this.awareness.clientID)
                        ownClock = Math.max(ownClock, clock);
                }
            }
        }
        catch {
    ...
    _ownPresenceClock(message) {
        const clockIn = (m) => {
            if (m[0] !== MESSAGE_AWARENESS)
                return -1;
        ...
        if (message[0] !== MESSAGE_BATCH)
            return clockIn(message);
        const decoder = decoding.createDecoder(message);
        decoding.readVarUint(decoder);
        let clock = -1;
        while (decoding.hasContent(decoder))
            clock = Math.max(clock, clockIn(decoding.readVarUint8Array(decoder)));
        return clock;
    }
    ...
    _send(data) {
        ...
        if (ownClock > this._presenceHeardClock)
            this._presenceHeardClock = ownClock;
