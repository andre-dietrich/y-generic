...
export class SimplePeerTransport {
    ...
    sendToPeer(peerConn, data) {
        ...
        this.log(`Chunking message: ${data.length} bytes into ${totalChunks} chunks`);
        ...
        return new Promise((resolve, reject) => {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    this.log('📩 Received:', msg.type, msg.from ? `from: ${msg.from}` : '', msg.to ? `to: ${msg.to}` : '', msg.signal ? `signal: ${msg.signal.type || 'candidate'}` : '');
                    this.handleSignalingMessage(msg);
                }
                catch (error) {
                    this.log('Error parsing signaling message:', error, 'Raw:', event.data);
                }
            };
            ws.onerror = (error) => {
            ...
            case 'publish':
                ...
                if (msg.from) {
                    ...
                    if (!this.peers.has(msg.from) &&
                        ...
                        this.log('Initiator tiebreaker:', this.peerId, '>', msg.from, '=', shouldInitiate);
    ...
    createPeerConnection(remotePeerId, initiator) {
        ...
        peer.on('signal', (signal) => {
            this.log('📤 Sending signal to peer:', remotePeerId, 'type:', signal.type || 'candidate');
                ...
                if (msgType === MSG_TYPE_COMPLETE) {
                    ...
                    if (buffer.chunks.size === totalChunks) {
                        ...
                        const decryptedData = this.options.password
                            ...
                            : fullMessage;
                        this._callback(decryptedData);
                        this.log(`Reassembled chunked message: ${totalLength} bytes from ${totalChunks} chunks`);
                    }
                }
                else {
    ...
    handlePeerSignal(remotePeerId, signal) {
        this.log('⬇️ handlePeerSignal from:', remotePeerId, 'type:', signal?.type || 'candidate');
