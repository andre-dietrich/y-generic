...
export class GenericProvider extends Observable<string> {
    ...
    const tick = () => {
      ...
      if (this._ownsAwareness) {
        ...
        if (remove.length > 0 && slept) {
          for (const id of remove) this.awareness.meta.get(id)!.lastUpdated = now
          woke = true
      ...
      if (woke && this.transport.isConnected && !this._destroying) this.syncNow()
  ...
  private _handlePeerLeave(peerId: string): void {
    ...
    if (this._peerAddress.size === 0) this._cancelPendingAwarenessRemoval()
  }
  ...
  private _schedulePeerConnectSync(peerId?: string): void {
    ...
    this._pendingPeerConnectSyncTimeoutId = setTimeout(() => {
      ...
      if (typeof this.transport.sendTo === 'function' && ids.length > 0) {
        ...
        if (state !== null) {
          if (this._ownsAwareness) {
            ...
            const mine = this.awareness.meta.get(this.doc.clientID)
            if (mine !== undefined && renewedAt !== undefined) mine.lastUpdated = renewedAt
          }
          messages.push(this._encodeAwareness([this.doc.clientID]))
        }
        const frame = wrapMessageWithChecksum(
          messages.length === 1 ? messages[0] : this._encodeBatch(messages),
        )
        for (const id of ids) this._sendToTransport(frame, id)
        return
      }
      ...
      for (const id of Array.from(this.awareness.meta.keys())) {
        if (id !== this.doc.clientID && !this.awareness.getStates().has(id)) this.awareness.meta.delete(id)
      }
      const state = this.awareness.getLocalState()
      if (state !== null && this._ownsAwareness) this.awareness.setLocalState(state)
      this._syncNow(DIGEST_FLAG_JOIN)
  ...
  private _dispatchMessage(message: Uint8Array, from?: string): void {
    ...
    switch (messageType) {
      ...
      case MESSAGE_AWARENESS: {
        ...
        if (scan.coversUs) this._presencePending.clear()
        ...
        if (
          ...
          if (vetoed.length > 0) update = this._withoutAwarenessEntries(payload, vetoed)
        }
  ...
  private _schedulePresenceResponse(requester: number): void {
    ...
    this._presenceResponseTimer = setTimeout(() => {
      ...
      this._presencePending.clear()
      if (requesters.length === 0) return // a relayed table answered them all
      if (this._destroying || !this.transport.isConnected) return
  ...
  private _sendUpdate(update: Uint8Array): void {
    ...
    if (!this._verifyUpdates) {
      ...
      for (const message of awareness) this._send(message)
      return
    }
  ...
  private _provesPresence(message: Uint8Array): boolean {
    ...
    if (message[0] !== MESSAGE_BATCH) return proves(message)
    const decoder = decoding.createDecoder(message)
    decoding.readVarUint(decoder)
    while (decoding.hasContent(decoder)) {
      if (proves(decoding.readVarUint8Array(decoder))) return true
    }
    return false
  }
