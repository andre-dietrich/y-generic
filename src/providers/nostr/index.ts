 ...
 */
function wrapFrame(data: Uint8Array): Uint8Array {
...
export class NostrTransport implements Transport {
  ...
  private snapshotSub: { close(): void } | null = null
  private snapshotChunks = new ChunkAssembler()
  private _onDocUpdate = (): void => this._queueSnapshotPublish()
  ...
  async connect(config: NostrConfig): Promise<void> {
    ...
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      const visible = () => {
        if (document.visibilityState === 'visible') this._resubscribeNow()
      }
    ...
    if (this.persistentMode) {
      ...
      this.snapshotSub = this.pool.subscribeMany(
        this.relays,
        { kinds: [this.persistentKind], '#d': snapshotDTags },
        {
  ...
  private _resubscribeNow(): void {
    ...
    for (const url of waiting) {
      ...
      if (this._debug) console.log('[NostrTransport] Page is back - subscribing now', url)
      this._subscribe(url)
    }
  }
