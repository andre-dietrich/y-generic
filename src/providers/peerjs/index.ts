...
export class PeerJSTransport implements Transport {
   ...
   */
  private rejoin(room: string, attempt: number): void {
    this.disconnect()
    const gen = ++this._rejoinGen
    this.connect({ room }).catch((err) => {
      if (gen !== this._rejoinGen) return // the app disconnected, or another re-join runs
      ...
      setTimeout(() => {
        if (gen === this._rejoinGen) this.rejoin(room, attempt + 1)
      }, delay)
    })
  }
