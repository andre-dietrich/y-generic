 ...
 */
export function watchNetworkChange(onChange: (why: string) => void): () => void {
  ...
  const online = () => {
    ...
    wasOffline = false
    if (!connection?.type) fire('the network is back') // with a type, `change` says it
  }
  window.addEventListener('offline', offline)
  window.addEventListener('online', online)
  let type: string | undefined = connection?.type
  const changed = () => {
    const now = connection?.type as string | undefined
    if (!now || now === 'none' || now === type) return
    const from = type
    type = now
    wasOffline = false
    if (from !== undefined) fire(`the network changed from ${from} to ${now}`)
  }
